/**
 * BookingWizard — reusable 5-step booking flow.
 * Used by BookingPage (/book/:username) and EmbedPage (/embed/:username).
 *
 * Steps: Session Type → Date → Time → Details → Confirmation
 */
...
export interface BookingWizardProps {
  ...
  onComplete?: (data: { bookingToken: string; guestName: string; startsAt: string }) => void;
  onClose?: () => void;
...
function makeInitialState(timezone: string): WizardState {
  return {
    ...
    slotLoading: false,
    selectedSlot: null,
    guestDetails: { guestName: '', guestEmail: '', timezone, notes: '' },
    submitting: false,
    submitError: null,
    bookingResult: null,
    message: null,
...
function wizardReducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    ...
    case 'PRACTITIONER_ERROR':
      return { ...state, practitionerLoading: false, practitionerError: action.message };
    ...
    case 'SLOTS_LOADED':
      return { ...state, slotLoading: false, slots: action.slots, slotState: action.state };

    case 'SELECT_SLOT':
      return { ...state, selectedSlot: action.slot, step: 'details' };
...
export default function BookingWizard({
  ...
  onClose,
  onError,
  // onResize is accepted but EmbedPage handles resize via ResizeObserver on its wrapper
  onResize: _onResize,
}: BookingWizardProps) {
  const { timezone, setTimezone, isDetected, timezoneList } = useTimezone();

  const [state, dispatch] = useReducer(wizardReducer, timezone, makeInitialState);
    ...
    (async () => {
      try {
        ...
        if (pErr || !practitioner) {
          dispatch({ type: 'PRACTITIONER_ERROR', message: 'This practitioner was not found.' });
          return;
        }
        ...
        if (stResult.error) {
          dispatch({ type: 'PRACTITIONER_ERROR', message: 'Failed to load session types. Please try again.' });
          return;
        }
        ...
        if (!cancelled) {
          dispatch({ type: 'PRACTITIONER_ERROR', message: 'Something went wrong. Please try again.' });
        }
      }
    })();
  ...
  useEffect(() => {
    ...
    if (preSelectedSessionTypeId) {
      const found = types.find((t) => t.id === preSelectedSessionTypeId);
      if (found) {
  ...
  // ── Fire callbacks when booking completes ─────────────────────────────────
  useEffect(() => {
    if (!state.bookingResult) return;
    if (onComplete) {
      onComplete({
        bookingToken: state.bookingResult.booking_token,
        guestName: state.guestDetails.guestName,
        startsAt: state.bookingResult.starts_at,
      });
    }
  }, [state.bookingResult]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Fire onError when submit error occurs ─────────────────────────────────
  useEffect(() => {
    if (!state.submitError) return;
    if (onError) {
      onError(state.submitError);
    }
  }, [state.submitError]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Booking submission ────────────────────────────────────────────────────
  const handleSubmitBooking = useCallback(async () => {
    if (!state.selectedSessionType || !state.selectedSlot || !state.practitioner || !username) return;
  ...
  if (!state.practitioner.is_active) {
    return (
      <div style={accentStyle}>
        <PractitionerHeader practitioner={state.practitioner} embed={embed} onClose={onClose} />
        <EmptyState
  ...
  return (
    <div style={accentStyle}>
      ...
      {state.step === 'session-type' && (
        <StepSection heading="What type of session?" headingRef={stepHeadingRef}>
          {state.sessionTypes.length === 0 ? (
            <EmptyState title="No sessions available" description="No session types are currently available." />
          ) : (
        ...
        >
          <div className="flex justify-center">
            <DayPicker
              ...
              onSelect={(date) => {
                if (!date) return;
                dispatch({ type: 'SELECT_DATE', date, dateStr: formatDateToISO(date) });
              }}
              disabled={[
                { before: minDate },
                { after: maxDate },
      ...
      {state.step === 'time' && state.selectedDateStr && (
        <StepSection
          ...
          onBack={() => dispatch({ type: 'GO_BACK' })}
        >
          <p className="text-sm text-gray-500 mb-4">{formatDisplayDate(state.selectedDateStr)}</p>
          <TimeSlotGrid
            slots={state.slots}
      ...
      {/* ── Step 4: Details ──────────────────────────────────────────── */}
      {state.step === 'details' && (
        <StepSection heading="Your details" headingRef={stepHeadingRef}>
          <GuestDetailsForm
            ...
            onSubmit={(details) => dispatch({ type: 'SUBMIT_DETAILS', details })}
            onBack={() => dispatch({ type: 'GO_BACK' })}
          />
        </StepSection>
      )}
    ...
    <div className="flex items-center gap-4 mb-6">
      ...
      {embed && onClose && (
        <button
          type="button"
          onClick={onClose}
          className="ml-2 shrink-0 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
